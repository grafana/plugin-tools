...
describe('startServer', () => {
  ...
  it('should serve static assets from docs folder', async () => {
    const result = await startServer({ docsPath: testDocsPath, port: 0 });
    server = result;
    app = result.app;
  ...
  it('should serve a nested directory child page', async () => {
    const result = await startServer({ docsPath: testDocsPath, port: 0 });
    server = result;
    app = result.app;

    const response = await request(app).get('/config/settings');
  ...
  it('should render headings in the table of contents sidebar', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.text).toContain('class="docs-toc"');

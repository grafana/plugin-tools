...
export const AGENT_RUNS_DIR = join('node_modules', '.cache', 'grafana-create-plugin', 'agent-runs');
...
export function getRunDir(basePath: string, runId: string): string {
  return join(basePath, AGENT_RUNS_DIR, runId);
}

export function getHandoffPath(runDir: string, stepName: string): string {
  return join(runDir, `${stepName}.json`);
}
...
export function wipeAgentRuns(basePath: string): void {
  rmSync(join(basePath, AGENT_RUNS_DIR), { recursive: true, force: true });
}

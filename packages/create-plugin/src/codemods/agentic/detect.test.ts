...
describe('resolveAgentFromPath', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    accessMock.mockResolvedValue(undefined);
  });

  it('should resolve the definition from the binary name', async () => {
    const definition = createDefinition({});

    const agent = await resolveAgentFromPath('/opt/homebrew/bin/claude', [definition]);

    expect(agent.definition).toBe(definition);
    expect(agent.binaryPath).toBe('/opt/homebrew/bin/claude');
  });

  it('should check the path is executable', async () => {
    await resolveAgentFromPath('/opt/homebrew/bin/claude', [createDefinition({})]);

    expect(accessMock).toHaveBeenCalledWith('/opt/homebrew/bin/claude', constants.X_OK);
  });

  it('should throw when the binary name matches no known agent', async () => {
    await expect(resolveAgentFromPath('/usr/local/bin/mystery', [createDefinition({})])).rejects.toThrow(
      /Could not tell which agent/
    );
  });

  it('should name the expected binaries when the name matches nothing', async () => {
    await expect(resolveAgentFromPath('/usr/local/bin/mystery', [createDefinition({})])).rejects.toThrow(/claude/);
  });

  it('should throw when the path is not executable', async () => {
    accessMock.mockRejectedValue(new Error('EACCES'));

    await expect(resolveAgentFromPath('/opt/homebrew/bin/claude', [createDefinition({})])).rejects.toThrow(
      /not an executable file/
    );
  });

  it('should not check executability when the name matches nothing', async () => {
    await expect(resolveAgentFromPath('/usr/local/bin/mystery', [createDefinition({})])).rejects.toThrow();

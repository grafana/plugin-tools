...
describe('run dir lifecycle', () => {
  ...
  it('should build run dirs under the agent-runs cache dir', () => {
    expect(getRunDir(basePath, 'run-1')).toBe(join(basePath, AGENT_RUNS_DIR, 'run-1'));
  });
  ...
  it('should wipe all previous runs', () => {
    ...
    expect(existsSync(join(basePath, AGENT_RUNS_DIR))).toBe(false);
  });

  it('should not throw when wiping a base path with no runs', () => {
    expect(() => wipeAgentRuns(basePath)).not.toThrow();
  });
});

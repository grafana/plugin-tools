...
3. Group diagnostics by `rule`. Apply fixes per category:
   ...
   | `has-markdown-files` / `*-index` / `*-naming`                    | Create missing required page from the same scaffold conventions used by `create-plugin add panel-docs`. Rename misplaced files. |

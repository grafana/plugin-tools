...
- The README typically stays in place after bootstrap. It still appears on npm and GitHub, but no longer on the plugin catalog page once `docsPath` is set - `index.md`'s Overview tab replaces it there. Bootstrap copies relevant README content onto pages; it does not delete the source file.

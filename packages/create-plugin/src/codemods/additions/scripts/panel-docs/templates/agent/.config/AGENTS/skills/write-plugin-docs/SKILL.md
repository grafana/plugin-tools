...
Run from the plugin root. The page path can be an existing doc page (e.g. `<docsPath>/options.md`) or a topic the user wants documented for the first time (e.g. "dashboard variable support").
...
2. **Check whether the new topic is tightly coupled to an existing page.** If so, restructure as a folder rather than creating a flat sibling. For example, if you are creating `tooltip.md` and `<docsPath>/options.md` already exists with overlapping scope, restructure both into:
   - `<docsPath>/options/index.md` (the existing options content moves here)
   - `<docsPath>/options/tooltip.md` (the new page)

...
vi.mock(import('../../utils.js'), async (importOriginal) => {
  ...
  const files = [
    ...
    '.config/app-sdk/generate-api-client.mjs',
  ...
  const rendered: Record<
    string,
    Record<'true' | 'false', { withoutBackend: string; withBackend: string }>
  > = Object.fromEntries(
    files.map((file) => [
      file,
      {
...
describe('experimental-app-sdk addition', () => {
  ...
  describe('scaffolding', () => {
    ...
    it('marks generate-kinds.mjs as scaffolded but leaves the editable CUE kinds unmarked', () => {
      ...
      expect(result.getFile('.github/workflows/generate-kinds-drift.yml')).not.toContain(
    ...
    it('does not clobber an existing generate:kinds script', () => {
      ...
      // The others are still added around it.
      expect(packageJson.scripts['generate:api-client']).toBe('node ./.config/app-sdk/generate-api-client.mjs');
    });

    it('adds the generate:api-client and generate scripts', () => {
      const context = createAppContext();
      ...
      const packageJson = JSON.parse(result.getFile('package.json') ?? '{}');
      expect(packageJson.scripts['generate:api-client']).toBe('node ./.config/app-sdk/generate-api-client.mjs');
      expect(packageJson.scripts.generate).toMatch(/run generate:kinds && .* run generate:api-client$/);
      expect(result.getFile('.config/app-sdk/generate-api-client.mjs')).toContain('write-openapi');
      expect(result.getFile('.config/app-sdk/generate-api-client.mjs')).toContain('grafana-api-clients');
    });

    it('adds the dependencies the generated API clients import', () => {
      const context = createAppContext();
    ...
    it('gitignores the intermediate OpenAPI documents', () => {
      const context = createAppContext();

      const result = appSdk(context);

      expect(result.getFile('.gitignore')).toBe('node_modules/\ndist/\n.config/app-sdk/openapi/\n');
    });

    it('does not duplicate the gitignore entry', () => {
      const context = createAppContext();
      context.updateFile('.gitignore', 'node_modules/\n.config/app-sdk/openapi/\n');
    ...
    it('checks the API clients for drift alongside the kinds', () => {
      const context = createAppContext();

      const result = appSdk(context);

      const workflow = result.getFile('.github/workflows/generate-kinds-drift.yml') ?? '';
      expect(workflow).toContain('src/api/generated');
      expect(workflow).toContain('run generate\n');
    });
...
`;
  ...
  describe('Go backend wiring', () => {
    ...
    it('does not duplicate the wiring on a re-run', () => {
      ...
      expect(
        (context.getFile('pkg/main.go') ?? '').match(/sdkplugin "github\.com\/grafana\/grafana-app-sdk\/plugin"/g)
      ).toHaveLength(1);
    });

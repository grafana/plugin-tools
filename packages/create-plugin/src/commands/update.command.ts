...
export const update = async (argv: minimist.ParsedArgs) => {
  if (!(await isGitDirectory()) && !argv.force) {
    output.error({
      ...
      body: [
        `In order to proceed please run ${chalk.yellow('git init')} in the root of your project and commit your changes.`,
        ...
        `In case you want to proceed as is please use the ${chalk.yellow('--force')} flag.)`,
      ],
    });
  ...
  if (!isPluginDirectory() && !argv.force) {
    output.error({
      ...
      body: [
        ...
        `(Please make sure to run this command from the root of your plugin folder. In case you want to proceed as is please use the ${chalk.yellow(
          '--force'
        )} flag.)`,
      ],
    });

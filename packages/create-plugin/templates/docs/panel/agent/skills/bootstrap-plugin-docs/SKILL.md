...
7. After all pages are written, run `npm run docs:validate` and fix what it reports. Repeat until clean.
...
- The README stays in place and remains the Overview tab's source, whether or not `docsPath` is set. `index.md` never appears on Overview - it is the Documentation tab's landing page, written for a reader who has already installed the plugin and clicked through from Overview. Bootstrap copies relevant README content onto pages; it neither deletes nor replaces the README.

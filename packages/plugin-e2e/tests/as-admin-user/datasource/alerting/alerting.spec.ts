...
test.describe('Alert Rule Query', () => {
  test.beforeAll(async ({ grafanaVersion }) => {
    test.skip(
      semver.lt(grafanaVersion, '9.5.0'),
      'Alerting rule test API are only compatible with Grafana 9.5.0 and later'
    );
  });

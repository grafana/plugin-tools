...
If you are using `@grafana/toolkit` to scaffold and develop plugins, follow this guide to make the jump to our newest plugin tools.
...
## Why migrate to the `create-plugin` tool?
...
If the plugin had customizations to the toolkit configs (for example, webpack), then refer to the [advanced configuration guide](./advanced-configuration.md) to learn how to update them.

If the plugin can be built, but there are problems with Jest tests, then see the [Jest advanced configuration](advanced-configuration.md#esm-errors-with-jest) issue regarding es modules.

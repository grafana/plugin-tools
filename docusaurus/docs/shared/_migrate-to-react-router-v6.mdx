...
#### 5. Troubleshooting testing routes

If you are using `@grafana/runtime` `locationService.replace()`, `locationService.push()` etc your tests may fail with React Router v6. To solve this you can take advantage of the `<LocationServiceProvider />` (added to Grafana in 11.2.0) to create a Provider for the locationService.

```typescript title="test-utils.tsx"

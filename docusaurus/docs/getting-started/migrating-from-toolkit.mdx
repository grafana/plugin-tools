...
If the plugin had customizations to the toolkit configs (for example, webpack), then refer to the [advanced configuration guide](../advanced-usage/advanced-configuration.md) to learn how to update them.

If the plugin can be built, but there are problems with Jest tests, then see the [Jest advanced configuration](/advanced-usage/advanced-configuration.md#esm-errors-with-jest) issue regarding es modules.

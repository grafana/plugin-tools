...
To adopt an optional feature in an existing plugin, run the `add` command from your plugin's root directory:

<AddNPM />

Run `add` without an addition name to list the additions available in your version of the tool.

Unlike `update`, which applies every pending change for your version, `add` applies one named change that you choose. The command stops if there are uncommitted changes in the repository. Use `--force` to override this.

For more information, refer to [Add features to your plugin](../how-to-guides/add-features-to-a-plugin.md).

### add --agent

Some additions include instructions for an AI coding agent, because the change depends on your plugin's own source code. If you have a supported agent CLI installed, `create-plugin` asks whether to use it. To select an agent without the question, use the `--agent` flag:

<AddAgentNPM />

The flag accepts an agent identifier, either `claude-code` or `codex`. It also accepts a path, which lets you use an agent that is installed outside your `PATH`:
...
The command then applies the automated part of the addition, if there is one, and prints the path to the instructions file so that you can follow it yourself.

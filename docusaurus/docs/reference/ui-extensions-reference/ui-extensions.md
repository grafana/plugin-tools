...
| **`category?`**     | **Deprecated: Use `group` instead.** A string used to group your link with other links.                                                                                                                                            | false    |
| **`group?`**        | A group used to organize the link with other links. Accepts an object with a required `name` (`string`) and an optional `icon` ([`IconName`](https://github.com/grafana/grafana/blob/main/packages/grafana-data/src/types/icon.ts#L1)). <br /> _Example: `{ name: "My group", icon: "bookmark" }`_ | false    |

...
1. **Defaults** apply to every field in the panel. Users set these in the panel editor under **All > Field options**.
2. **Overrides** apply only to fields that match a specific rule. Users add these under **All > Overrides**, where each rule has a _matcher_ (which fields to target) and one or more _properties_ (what to change).
...
Users can match fields by exact name, by regular expression, by field type, or by the query that returned them.
...
- **`shouldApply`**: Restricts the property to specific field types. When set, the property is hidden from the override picker for non-matching fields.

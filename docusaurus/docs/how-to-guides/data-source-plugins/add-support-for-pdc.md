...
Note: It is not possible to add PDC support to frontend data sources because connection to the proxy is established from the backend.
:::
...
In this case, we will use [microsocks](https://github.com/rofl0r/microsocks), which is an OSS Socks5 server. This will not be the same as how Grafana runs in our cloud, but it shows a simpler way to do this without any internal dependency.

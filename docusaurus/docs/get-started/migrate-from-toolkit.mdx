...
If the plugin had customizations to the toolkit configs (for example, webpack), then refer to the [advanced configuration guide](/plugin-tools/docs/create-a-plugin/extend-configurations) to learn how to update them.

If the plugin can be built, but there are problems with Jest tests, then see the [Jest advanced configuration](/plugin-tools/docs/create-a-plugin/extend-configurations#esm-errors-with-jest) issue regarding es modules.

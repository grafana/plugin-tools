...
1. When prompted, enter answers to the given questions. For example:
  ...
  If you have previously built a plugin with `@grafana/toolkit`, you can use our plugin tools to simplify migration. For more information, refer to [Migrate from toolkit](../migration-guides/migrate-from-toolkit).
...
Add [Github workflows](/docs/create-a-plugin/set-up-github-workflows) to your development cycle to help catch issues early or release your plugin to the community.

### Do you want to add a Github workflow to automatically check Grafana API compatibility on PRs?

Add a [Github workflow](/docs/create-a-plugin/set-up-github-workflows#the-compatibility-check-is-compatibleyml) to regularly check that your plugin is compatible with the latest version of Grafana.

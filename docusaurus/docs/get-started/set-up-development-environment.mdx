...
1. <SyncCommand cmd="run server" />: Starts a Grafana development server running on
   [http://localhost:3000](http://localhost:3000). This command will run a `mage -v watch` inside the container
   and the plugin will be reloaded on every code change. It also allows you to attach a debugger to the plugin backend code on port `2345`.

...
## Plugin backend v1 support has been dropped

Use the new [plugin SDK for Go](../../key-concepts/backend-plugins/grafana-plugin-sdk-for-go) to run your plugin backend running in Grafana 8.
...
## Sign and load plugins with a backend component
...
Grafana uses the Emotion library to manage frontend styling. We've updated the Emotion package and this can affect your plugin frontend component if you have custom styling. You only need to update the `import` statements to get it working in Grafana 8.

...
As of Grafana 10.0, `@grafana/toolkit` is deprecated. It is replaced by the [`create-plugin`](../../get-started.md) tool.
...
Since Grafana 7.0 introduced signing of plugins, community plugins won’t load by default if they’re unsigned.
...
As of Grafana 7.0, plugins can now be cryptographically signed to verify their origin. By default, Grafana ignores unsigned plugins. For more information, refer to [Allow unsigned plugins](https://grafana.com/docs/grafana/latest/administration/plugin-management#allow-unsigned-plugins).

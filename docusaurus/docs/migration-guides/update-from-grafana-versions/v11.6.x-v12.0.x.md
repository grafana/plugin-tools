...
Props to overriding rendering are no longer supported in order to simplify the API and keep it fast.

Many props used to control subtle behaviour, such as `createOptionPosition` or `openMenuOnFocus` are no longer supported to simplify the API and have a more consistent experience. If you have a specific use case that requires this, please open an issue in the Grafana repo.

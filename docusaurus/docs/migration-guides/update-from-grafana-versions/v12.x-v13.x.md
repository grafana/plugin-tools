...
:::warning
Applying this fix will make your plugin incompatible with the following versions of Grafana:

- All versions of Grafana before `11.6.11`
- `12.0.0 - 12.0.9`
- `12.1.0 - 12.1.6`
- `12.2.0 - 12.2.4`
  :::

To solve this issue automatically you can run the following command which will update the webpack config and adjust your plugin to only support specific versions of Grafana.

```shell npm2yarn
npx @grafana/create-plugin@latest add externalize-jsx-runtime
```

Should the above command fail the following steps will guide you through manually externalizing the jsx-runtime modules in your plugin's webpack configuration and update the plugin.json file.
